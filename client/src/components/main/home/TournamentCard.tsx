...
export default function TournamentCard({
  ...
  // Helper to format date string in the required format: OCT 07, 5:10 AM
  ...
  return (
    <div
      className={`w-full sm:w-72 md:w-80 rounded-2xl overflow-hidden bg-zinc-900 group text-white transition-all duration-300 border ${
        isHovered ? 'border-orange-500 shadow-lg shadow-orange-900/20' :
        isHighlighted ? 'border-orange-500' : 'border-gray-800'
      }`}
    ...
    >
      ...
      <div className="relative h-46 overflow-hidden">
        ...
        {tournament.isPlaying && (
          <div className={`absolute bottom-0 left-4 text-xs px-3 py-1 rounded-full flex items-center group-hover:bg-orange-500 ${isHovered ? ' bg-orange-500 text-white transform-transition duration-300' : "border border-green-500 text-white bg-gray-950"}`}>
      ...
      <div className="px-4 py-4">
        {/* Title */}
        <h3 className={`font-furore text-md mb-1 ${isHighlighted || isHovered ? 'text-orange-500 transform duration-300 transition-all' : 'text-white'}`}>
          {tournament.title}
        </h3>
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <div className="flex items-center">
            <div className="bg-yellow-500 text-white w-4 h-4 rounded-full flex items-center justify-center mr-2">
              <span className="text-xs font-bold"><BiRupee /></span>
            </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center text-xs text-gray-400">
            <Users size={16} className={`mr-2 ${isHovered ? "text-orange-500" : "text-gray-500"} `} />
            <span>{tournament.currentTeams}/{tournament.maxTeams} Teams</span>
          </div>

          {/* Players */}
          <div className="text-xs flex text-gray-400">
            <User size={16} className={`mr-2 ${isHovered ? "text-orange-500" : "text-gray-500"} `} />
          ...
          <button
            ...
            className={`w-8 h-8 rounded-full ${isHovered ? 'bg-orange-500' : 'bg-green-500'} flex items-center justify-center transition-all`}
          >
