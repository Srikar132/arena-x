...
const GameCardSvg:React.FC<GameCardSvgProps> = ({
  ...
  return (
    <div
      className={`flex flex-col items-center ${className} relative transition-transform duration-300 ${isHovered ? '' : ''}`}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      onClick={onClick}
    >
      {/* SVG Card */}
      <svg xmlns="http://www.w3.org/2000/svg" height="75%" viewBox="0 0 300 450" className="w-full max-w-xs cursor-pointer">
        <defs>
          ...
          <clipPath id={imageClipId}>
            <path d={`M${40+padding},${padding} H${280-padding} L${300-padding},${20+padding} V${430-padding} L${260-padding},${450-padding} H${20+padding} L${padding},${430-padding} V${40+padding} L${40+padding},${padding} Z`} />
          </clipPath>
        ...
        {/* Clipped content (image inside the border with padding) */}
      ...
      <h3
        className={`font-furore text-xl md:text-2xl mt-4 tracking-wider transition-colors duration-300 ${
      ...
      <div className='absolute group top-0 left-0 right-0 bottom-28 flex-center'>
        <Link to = "/games">
          <button title='click' className = "w-12 h-12 hidden group-hover:flex -rotate-45 items-center justify-center rounded-full bg-orange-500 p-2">
